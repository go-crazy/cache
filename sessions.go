...
func (s *Sessions) updateCookie(ctx *Gin.Context, sid string, expires time.Duration) {
	cookie := &http.Cookie{}
...
func (s *Sessions) Start(ctx *Gin.Context) *Session {
...
func (s *Sessions) Cache(key string) *Session {

	if key == ""  { // cookie doesn't exists, let's generate a session 
		...
		log.Fatalf("Cache by a error key!")
		return sess
	}

	sess := s.provider.Read(key, s.config.Expires)

	return sess
}
// ShiftExpiration move the expire date of a session to a new date
// by using session default timeout configuration.
func (s *Sessions) ShiftExpiration(ctx *Gin.Context) {
...
func (s *Sessions) UpdateExpiration(ctx *Gin.Context, expires time.Duration) {
	cookieValue := s.decodeCookieValue(GetCookie(ctx, s.config.Cookie))
...
func (s *Sessions) Destroy(ctx *Gin.Context) {
