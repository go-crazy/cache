...
import (
	...
	"runtime"
	"log"
	"github.com/go-crazy/cache/errors"
...
func New(directoryPath string) (*Database, error) {
	...
	if err != nil {
		log.Fatalf("unable to initialize the badger-based session database: %v", err)
		return nil, err
	}
...
func (db *Database) Load(sid string) (storeDB sessions.RemoteStore) {
	...
	if err != nil {
		log.Fatalf("error while trying to get the serialized session(%s) from the remote store: %v", sid, err)
		return
	}

	storeDB, err = sessions.DecodeRemoteStore(b) // decode the whole value, as a remote store
	if err != nil {
		log.Fatalf("error while trying to load from the remote store: %v", err)
	}
...
func (db *Database) sync(p sessions.SyncPayload) {
	...
	if p.Action == sessions.ActionDestroy {
		if err := db.destroy(bsid); err != nil {
			log.Fatalf("error while destroying a session(%s) from badger: %v",
				p.SessionID, err)
		}
		return
	}
	...
	if err != nil {
		log.Fatalf("error while serializing the remote store: %v", err)
	}
	...
	if err != nil {
		txn.Discard()
		log.Fatalf("error while trying to save the session(%s) to the database: %v", p.SessionID, err)
		return
	}
	if err := txn.Commit(nil); err != nil { // Commit will call the Discard automatically.
		log.Fatalf("error while committing the session(%s) changes to the database: %v", p.SessionID, err)
	}
}
...
func closeDB(db *Database) error {
	...
	if err != nil {
		log.Fatalf("closing the badger connection: %v", err)
	}
	return err
}
