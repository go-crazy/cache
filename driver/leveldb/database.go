...
func New(directoryPath string) (*Database, error) {
	...
	if err != nil {
		log.Fatalf("unable to initialize the LevelDB-based session database: %v", err)
		return nil, err
	}
...
func (db *Database) Load(sid string) (storeDB sessions.RemoteStore) {
	...
	if err := iter.Error(); err != nil {
		log.Fatalf("error while trying to iterate over the database: %v", err)
	}
...
func (db *Database) sync(p sessions.SyncPayload) {
	...
	if p.Action == sessions.ActionDestroy {
		if err := db.destroy(bsid); err != nil {
			log.Fatalf("error while destroying a session(%s) from leveldb: %v",
				p.SessionID, err)
		}
		return
	}
	...
	if err != nil {
		log.Fatalf("error while writing the session(%s) to the database: %v", p.SessionID, err)
	}
}
...
func closeDB(db *Database) error {
	...
	if err != nil {
		log.Fatalf("closing the LevelDB connection: %v", err)
	}
