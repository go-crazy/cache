...
import (
	...
	"github.com/go-crazy/cache/driver/redis/service"
)
...
func (db *Database) Load(sid string) (storeDB sessions.RemoteStore) {
	...
	if !db.redis.Connected { //yes, check every first time's session for valid redis connection
		...
		if err != nil {
			log.Fatalf("redis database error on connect: %v", err)
			return
		}
	}
	...
	if err == nil {
		...
		if !ok {
			log.Fatalf("something wrong, store should be stored as []byte but stored as %#v", storeMaybe)
			return
		}

		storeDB, err = sessions.DecodeRemoteStore(storeB) // decode the whole value, as a remote store
		if err != nil {
			log.Fatalf(`error while trying to load session values(%s) from redis:
...
func (db *Database) sync(p sessions.SyncPayload) {
	...
	if err != nil {
		log.Fatalf("error while encoding the remote session store")
		return
	}
