...
func New(path string, fileMode os.FileMode, bucketName string) (*Database, error) {
	...
	if err := os.MkdirAll(filepath.Dir(path), fileMode); err != nil {
		log.Fatalf("error while trying to create the necessary directories for %s: %v", path, err)
		return nil, err
	}
	...
	if err != nil {
		log.Fatalf("unable to initialize the BoltDB-based session database: %v", err)
		return nil, err
	}
		...
		for k, v := c.First(); k != nil; k, v = c.Next() {
			...
			if storeDB.Lifetime.HasExpired() {
				if err := c.Delete(); err != nil {
					log.Fatalf("troubles when cleanup a session remote store from BoltDB: %v", err)
				}
			}
		}
...
func (db *Database) sync(p sessions.SyncPayload) {
	...
	if p.Action == sessions.ActionDestroy {
		if err := db.destroy(bsid); err != nil {
			log.Fatalf("error while destroying a session(%s) from boltdb: %v",
				p.SessionID, err)
		}
		return
	}
	...
	if err != nil {
		log.Fatalf("error while writing the session bucket: %v", err)
	}
}
...
func closeDB(db *Database) error {
	...
	if err != nil {
		log.Fatalf("closing the BoltDB connection: %v", err)
	}
